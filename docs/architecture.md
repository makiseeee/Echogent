...
- MCP 提供状态探测、受限 Python、白名单目录只读和受控 Obsidian 访问
...
```text
状态探测 -> 10.144.232.236:8765/status -> echo-executor
远程执行/Obsidian -> 10.144.232.236:8765/mcp -> echo-executor
...
| Obsidian Vault | 电脑 | 独立个人库，按目录与会话类型授权 |
| Ollama 模型 | 电脑 | `bge-m3` |
...
- MCP 离线：Agent 会获知电脑离线；远程执行和 Obsidian 工具不可用，不影响普通聊天
...
- Obsidian 新建和修改必须先预览，再由本人下一条消息明确确认
