"""Best-effort Git synchronization for a shared Obsidian vault with async push and pull debounce."""
...
class VaultGitSync:
    ...
    def _async_push_worker(self) -> None:
        with self._lock:
            try:
                self._git("push", self.remote, self.branch)
                LOG.info("Obsidian background git push completed successfully.")
            except (subprocess.SubprocessError, OSError) as exc:
                warning = self._message(exc)
                if "non-fast-forward" in warning.lower() or "rejected" in warning.lower():
                    ok, _ = self._pull_unlocked(force=True)
                    if ok:
                        try:
                            self._git("push", self.remote, self.branch)
                            LOG.info("Obsidian background git push retry succeeded.")
                        except Exception as e:
                            LOG.warning("Obsidian background git push retry failed: %s", e)
                else:
                    LOG.warning("Obsidian background git push failed; commit kept locally: %s", warning)

    def atomic_write(self, action_func: Callable[[], T], commit_message: str) -> tuple[T, GitSyncResult]:
        with self._lock:
            _, pull_warning = self._pull_unlocked(force=False)
            try:
                value = action_func()
            except Exception:
